    ...
    messages: [
      {
        role: "system",
        content:
          "You work on company generating daily mature fables to learn foreign language. Compact answer in requested format."
      },
      {
        role: "user",
        content:
          "Generate literal translations for below words. Can be multiple words for common synonyms. all words uses lowercase. \nWords: ['#{words.join("','")}']\nFrom: #{from_language}\nTo: #{to_language}\nFormat: JSON Map<string, string[]> (keyword to translations)"
      }
    ],
    temperature: 1,
    max_tokens: 256,
...
def exec(parsed_input, from_language, to_language, complexity)
  ...
  translated_metadata =
    JSON.parse(
      clean_json generate_metadata_translations(
                   LANG_CODE[from_language],
                   LANG_CODE[to_language],
                   complexity,
                   strmetadatas
                 )
    )
  ...
  if to_language != "en"
    translated_paragraphs =
      JSON.parse(
        clean_json generate_paragraph_translations(
                     LANG_CODE[to_language],
                     3,
                     strparagraphs
                   )
  ...
  keywords_translations =
    JSON.parse(
      clean_json generate_keywords_translations(
                   translated_paragraphs["keywords"],
                   LANG_CODE[to_language],
                   LANG_CODE[from_language]
                 )
    )
